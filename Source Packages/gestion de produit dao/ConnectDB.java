...
public class ConnectDB {
    ...
    public void insertFornisseur(Fornisseur forn) {
        ...
        try {
            st = cnx.createStatement();
            String sql1 = "INSERT INTO `fornisseur`( `nom_prenom`, `N_telephone`, `Adresse`,`nfix`) "
                    + "VALUES ('" + forn.nom_prenom + "'," + "'" + forn.telephone + "','" + forn.adresse + "','" + forn.nfix + "') ";
    ...
    public void insertClient(Client cl) {
        try {
            st = cnx.createStatement();
            String sql1 = "INSERT INTO `client`( `nom_prenom`, `Adresse`, `N_telephone`) "
                    + "VALUES ('" + cl.nom_prenom + "'," + "'" + cl.adresse + "','" + cl.telephone + "') ";
            st.executeUpdate(sql1);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, "ereur operation");
    ...
    public void AllFornisseur(JTable T) {
        try {
            ...
            while (rst.next()) {
                ...
                j++;
                T.setValueAt(rst.getString("nfix").toString(), i, j);
                j++;
                i++;
    ...
    public void UpdatForn(int id, Fornisseur fr) {
        ...
        try { st = cnx.createStatement();
            String sql = "Update fornisseur set nom_prenom='" + fr.nom_prenom + "',N_telephone='" + fr.telephone + "',"
                    + "Adresse='" + fr.adresse + "'" + ",nfix='" + fr.nfix + "'"
                    + " where N_forn=" + id;
            st.executeUpdate(sql);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void FindClient(JTable T, String word,String recherche) {
        try {
            st = cnx.createStatement();
            String sql = "SELECT * FROM `client` WHERE " + word + " like '" + recherche + "%'  ";
            rst = (ResultSet) st.executeQuery(sql);
            int i = 0, j;
            while (rst.next()) {
                j = 0;
    ...
    public void FindFornisseur(JTable T, String word, String recherche) {
        try {
            ...
            while (rst.next()) {
                ...
                j++;
                T.setValueAt(rst.getString("nom_prenom").toString(), i, j);
                j++;T.setValueAt(rst.getString("N_telephone").toString(), i, j);
                j++;
                  T.setValueAt(rst.getString("Adresse").toString(), i, j);
                j++;
    ...
    public void inserFacture(Facture fac) {
        try {
            float r = fac.montant_a_paye - fac.montant_verser;
            st = cnx.createStatement();
            String sql1 = "INSERT INTO `facture`( `nom_prenom_cl`, `montant_a_paye`, `montant_verser`"
                    + ",`montant_reste`, `date_versement`) "
                    + "VALUES ('" + fac.nom_prenom_cl + "'," + "'" + fac.montant_a_paye + "','" + fac.montant_verser + "'"
                    + ",'" + r + "','" + showDate() + "') ";
            st.executeUpdate(sql1);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void AllFacture(JTable T) {
        try {
            ...
            while (rst.next()) {
                Facture fac = new Facture(rst.getInt("N_fact"), rst.getString("nom_prenom_cl"),
                        rst.getFloat("montant_a_paye"), rst.getFloat("montant_verser"),
                        rst.getFloat("montant_reste"), rst.getDate("date_versement") + "");
                list.add(fac);
            }
            DefaultTableModel mo = (DefaultTableModel) T.getModel();
            mo.setRowCount(0);
            ...
            for (int k = 0; k < list.size(); k++) {
                ...
                row[1] = list.get(k).getnom_prenom_cl();
    ...
    public void FindPrixTotale(String s, JTextField jt) {
        try {
            ...
            String sql = "SELECT sum(prix_a_payez) FROM `vente` WHERE nom_prenom_client = '" + s + "'  ";
            st = cnx.prepareStatement(sql);
            rst = st.executeQuery(sql);
            if (rst.next()) {
                String f = rst.getString("sum(prix_a_payez)");
                jt.setText(f + "");
            }
    ...
    public void UpdatFact(int id, Facture fc) {
        ...
        try {
            ...
            String sql = "Update facture set nom_prenom_cl='" + fc.nom_prenom_cl + "',montant_a_paye='" + fc.montant_a_paye + "',"
                    + "montant_verser='" + fc.montant_verser + "'" + ",montant_reste='" + reste + "',"
                    + "date_versement='" + showDate()+ "'" + "where N_fact=" + id;
            st.executeUpdate(sql);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String FindPrixTotale() {
        String f = "";
        try {
            st = cnx.prepareStatement(url);
            String sql = "SELECT sum(montant_a_paye) FROM `facture`  ";
            st = cnx.prepareStatement(sql);
            rst = st.executeQuery(sql);
            if (rst.next()) {
                f = rst.getString("sum(montant_a_paye)");
    ...
    public void AllStock(JTable T) {
        ...
        try {
            ...
            st = cnx.createStatement();
            String sql = "select `N_prod`, `designation`, `date_exp`, `qte_totale`, `qte_vente`, `qte_reste`, `prix_unt`, `prix_vent`, `gain`, `categorie` from stock WHERE `unite`='0'";
            rst = (ResultSet) st.executeQuery(sql);
            DES.clear();
            while (rst.next()) {
...
//                row[4] = list.get(k).getDate_exp();
     ...
     public void AllStockar(JTable T) {    //table archivé

        try {
            ArrayList<Stock> list = new ArrayList<Stock>();
            st = cnx.createStatement();
            String sql = "select * from stock WHERE `unite`='1'";
            rst = (ResultSet) st.executeQuery(sql);
            DES.clear();
            while (rst.next()) {
...
//                row[4] = list.get(k).getDate_exp();
     ...
     public void AllStockM(JTable T) {   //table alarme

        try {
            ArrayList<Stock> list = new ArrayList<Stock>();
            st = cnx.createStatement();
            String sql = "select * from stock WHERE `qte_reste`<'10' AND `unite`='0'";
            rst = (ResultSet) st.executeQuery(sql);
            DES.clear();
            while (rst.next()) {
...
//                row[4] = list.get(k).getDate_exp();
    ...
    public ArrayList<String> CONTROLE2(JComboBox com) {
     ...
     public void insertStock(String sql) {
...
String s="";s= rst.getDate("date_vente")+""+" " +rst.getTime("date_vente")+""; 
        ...
        try {
          ...
          public void Maxvent(JTable T) {   //table alarme

        try {
            ArrayList<Stock> list = new ArrayList<Stock>();
            st = cnx.createStatement();
            String sql = "SELECT  `N_prod`,`designation`,`categorie`,MAX(qte_vente) as maxqte FROM `stock`";
            rst = (ResultSet) st.executeQuery(sql);
            DES.clear();
            while (rst.next()) {
                Stock ST = new Stock(rst.getInt("N_prod"), rst.getString("designation"),
                        rst.getString("categorie"),rst.getInt("maxqte"));
                list.add(ST);
            }
            DefaultTableModel mo = (DefaultTableModel) T.getModel();
            mo.setRowCount(0);
          ...
          public void gagner(JTable T) {   //table alarme

        try {
            ArrayList<Stock> list = new ArrayList<Stock>();
            st = cnx.createStatement();
            String sql = "SELECT SUM(gain) as gagner FROM `stock`";
            rst = (ResultSet) st.executeQuery(sql);
            DES.clear();
            while (rst.next()) {
                Stock ST = new Stock(rst.getInt("gagner"));
                list.add(ST);
            }
            DefaultTableModel mo = (DefaultTableModel) T.getModel();
            mo.setRowCount(0);
